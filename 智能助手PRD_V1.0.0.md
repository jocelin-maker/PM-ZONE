...
| V-1.0.1 | 2026-06-11 | XXX（产品） | 待评审 | 配套高保真**可交互原型**：基于本 PRD 实现登录/首页/角色/对话/历史/设置/异常全流程可点击演示，PRD 升级为「可读 + 可交互」 | [交互原型](./prototype/智能助手交互原型.html) |

> **状态流转说明**：草稿 → 待评审 → 已定稿 → 完成 → 更新
> **当前状态**：待评审（待研发、设计、测试三方评审后定稿）

> 🔗 **配套交互原型**：[`prototype/智能助手交互原型.html`](./prototype/智能助手交互原型.html)（单文件，双击用浏览器打开即可操作；右侧「演示控制台」可一键切换登录态与异常分支）。原型与本文档各章节、AC 验收点一一对应，详见 [附录 A](#附录-a交互原型说明)。
...
> 文件：[`prototype/智能助手交互原型.html`](./prototype/智能助手交互原型.html) — 单文件、零依赖，浏览器直接打开即可操作。左侧为手机模拟器（可直接点击操作全流程），右侧为「演示控制台」（用于一键切换登录态、注入异常分支、查看实时状态）。

### A.1 如何使用

1. 双击 HTML 文件用浏览器打开（推荐 Chrome / Edge / Safari）。
2. 在左侧手机内直接操作：点击输入框/热门话题 → 触发登录 → 选择角色 → 进入对话 → 提问、切换模式语调、点赞点踩、重新生成翻页、追问、新建/重命名/删除会话、设置改角色等。
3. 用右侧控制台快速演示分支：
   - **登录态**：「重置为未登录新用户」/「登录为老用户（已选角色）」。
   - **异常兜底**：敏感词阻断 / 网络超时 / 频率限制（20/分）/ 恢复正常。

### A.2 原型已实现能力 ↔ AC 映射

| 原型可操作点 | 对应章节 | 对应 AC |
| :--- | :--- | :--- |
| 未登录点击输入框/话题唤起登录 | 5.1 / 4.2 | AC-01 |
| 微信授权（已绑/未绑/失败三分支）+ 手机号验证码绑定 | 5.1 / 4.2 | AC-02 ~ AC-04 |
| 退出登录清态返回首页 | 5.1 | AC-05 |
| 角色定向推荐 / 未设角色与异常兜底 3 问 | 5.2 | AC-06 ~ AC-07 |
| 状态齐全携问进入对话并立即生成 | 5.2 / 4.3 | AC-08 |
| 首次注册强制弹角色（仅一次）/ 跳过提示 | 5.3 | AC-09 ~ AC-10 |
| 思考过程默认展开、可折叠、完成后自动收起（专家模式展示） | 5.4.2 | AC-11 |
| 快速+亲和小红书式 / 专家+专业学术式排版联动 | 5.4.2 | AC-12 ~ AC-13 |
| 同会话内切换模式/语调，历史轮次保持原形态 | 5.4.2 | AC-14 |
| 答案底部「引用 X 篇资料 [1][2]」 | 5.4.2 | AC-15 |
| 点赞/点踩互斥；点踩弹窗（类别必选 + 补充说明 1–300 字校验） | 5.4.2 | AC-16 ~ AC-18 |
| 一键复制 Toast | 5.4.2 | AC-19 |
| 重新生成（≤5 次 / 共 6 版本，超限隐藏）+ `<1/N>` 翻页（最新在前） | 5.4.2 | AC-20 ~ AC-22 |
| 回答后 3 个「猜你想问」胶囊，点击即发送 | 5.4.2 | AC-23 |
| 输入 1–1000 字校验（空置灰、满 1000 截断飘红）+ 流式中断「⏹ 停止生成」 | 5.4.2 | AC-24 ~ AC-25 |
| 上滑出现「回到最新」并定位 | 5.4.2 | AC-26 |
| 新建对话 / 倒序列表 / 自动命名（前 10 字 +…）/ 重命名（≤30 非空）/ 删除二次确认 | 5.5 | AC-27 ~ AC-31 |
| 设置内修改核心角色即时生效并影响推荐 | 5.6 | AC-32 |
| 敏感词阻断兜底文案 | 7 / EB-01 | AC-34 |
| 网络超时失败提示 + 重新生成入口 | 7 / EB-02 | AC-35 |
| 频率限制（每分钟 20 次）提示 | 7 / EB-03 | AC-36 |

### A.3 原型边界说明

- 原型为**前端交互演示**，回答内容为本地 Mock（非真实大模型），用于评审交互与流程，不代表最终文案与算法效果。
- 后台「核心角色」字段（5.7 / AC-33）属管理后台范畴，未纳入本 C 端原型。
- 数据不落库、刷新后重置；登录、验证码、推荐等均为模拟。
